...
FileMan::FileMan(const std::string& filename, const std::string& main){
	...
	std::ifstream file(main + "/" + filename);
...
void FileMan::set_output_path(const std::string& output){
	...
	std::string full_path_loc = get_main_dir();
	std::istringstream is(line);
	while (std::getline(ss, line, '/')){
		full_path_loc = full_path_loc + "/" + line;
		mkdir(full_path_loc.c_str(), 0777);
	}
	std::string pic_path = full_path_loc + "/pictures";
	mkdir(pic_path.c_str(), 0777); 
	
	output_path = get_main_dir() + "/data/geometry=" + get_geom_fn() + "/" + output;
}
